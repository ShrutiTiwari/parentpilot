...
**Why forward-not-connect** — Most AI calendar tools ask for full Gmail/Outlook OAuth access. Power Parent takes the opposite approach: parents forward exactly the emails they choose to share, one at a time. The agent sees only what the parent decides to show it. No inbox scanning. No background access. No data the parent didn't explicitly hand over. For parents who want more automation without sacrificing control, Gmail and Outlook both support auto-forward rules filtered by sender — so a parent can set "auto-forward anything from school@stmarys.co.uk" and never think about it again, while still keeping the rest of their inbox private. In a market moving fast toward ambient inbox access, this is a deliberate privacy-first design choice — and the right one for a product handling information about children.
